#!/usr/bin/env python3
"""AST-based test-quality linter for the MORS test suite.

Enforces the rules in `.github/.claude/rules/mors-tests.md` (sections 1 + 7):
...
- Forbidden weak assertions when they stand alone as the sole meaningful
  ...
  three-class discrimination guard from mors-tests.md section 2 uses
...
- Optional dependencies (``hypothesis``) imported at module top without a
  preceding ``pytest.importorskip('<name>')`` (the ``pip install --no-deps``
  CI image otherwise fails collection).
...
# Source rule: mors-tests.md section 5.
OPTIONAL_DEPS = {
    'hypothesis',
}

# Physics source files. Each one should have a companion test file under
# tests/ carrying at least one @pytest.mark.physics_invariant test and at
# least one @pytest.mark.reference_pinned test (mors-tests.md section 3).
PHYSICS_SOURCES = {
    'baraffe.py',
    'cluster.py',
    'physicalmodel.py',
...
def reference_pinned_status() -> list[str]:
    ...
    for source in sorted(PHYSICS_SOURCES):
        # source is e.g. 'baraffe.py' -> companion is tests/test_baraffe.py
...
def cmd_baseline() -> int:
    ...
    allow = os.environ.get('MORS_TEST_QUALITY_ALLOW_REGRESS') == '1'
    if old and new_total > old_total and not allow:
        print(
            ...
            f'pre-existing violations), set MORS_TEST_QUALITY_ALLOW_REGRESS=1.',
            file=sys.stderr,
        )
        return 2
