...
* fast  - updates `[tool.mors.coverage_fast].fail_under` (unit/smoke gate)
...
def read_threshold_from_pyproject(target: str) -> float:
    ...
    try:
        ...
        if target == 'fast':
            return float(data['tool']['mors']['coverage_fast']['fail_under'])
...
def update_threshold_in_pyproject(target: str, new_threshold: float) -> bool:
    ...
    if target == 'full':
        ...
        mors_section = document.get('tool', {}).get('mors')
        if mors_section is None:
