...
def Fig1():
    ...
    def UpperPanel():
        ...
        plt.xlabel(r'$\Omega_\mathrm{env} / \Omega_\mathrm{break}$')
    ...
    def LowerPanel():
        ...
        plt.xlabel('Age (Myr)')
        plt.ylabel(r'Break-up rotation rate ($\Omega_\odot$)')
        plt.xscale('log')
        plt.yscale('log')
...
def Fig3():
    ...
    def Panel(Mstar):
        ...
        plt.xlabel('Age (Myr)')
        plt.ylabel(r'Rotation rate ($\Omega_\odot$)')
        plt.xscale('log')
        plt.yscale('log')
        plt.xlim([1.0, 5000.0])
        plt.ylim([0.5, 300.0])
...
def Fig5():
    ...
    def MiddlePanel(Mstar, Age):
        ...
        plt.xlabel('Age (Myr)')
        plt.ylabel(r'Saturation rotation rate ($\Omega_\mathrm{sat}$)')
        plt.xscale('log')
        plt.yscale('log')
    ...
    def LowerPanel(Mstar, Age):
        ...
        for iMstar in range(len(Mstar)):
            for iAge in range(len(Age)):
                LxSat[iMstar, iAge] = mors.Lx(
                    Mstar=Mstar[iMstar], Age=Age[iAge], Omega=OmegaSat[iMstar, iAge]
                )
        ...
        plt.xlabel('Age (Myr)')
        plt.ylabel(r'Saturation $L_\mathrm{X}$ (erg s$^{-1}$ cm$^{-2}$)')
        plt.xscale('log')
        plt.yscale('log')
...
def Fig9():
    ...
    def LeftColumn():
        ...
        for i in range(len(Ages)):
            ...
            plt.ylabel(r'Rotation rate ($\Omega_odot$)')
            plt.xlim([0.08, 1.22])
            plt.ylim([0.1, 150.0])
    ...
    def MiddleColumn():
        ...
        for i in range(len(Ages)):
            ...
            for iMstar in range(len(Mstar)):
                LxSat[iMstar] = mors.Lx(
                    Mstar=Mstar[iMstar], Omega=OmegaSat[iMstar], Age=Ages[i]
                )
...
def Fig12():
    ...
    def LeftPanel():
        ...
        plt.xlabel(r'Stellar mass (M$_\odot$)')
        plt.ylabel(r'Active lifetime for $L_\mathrm{X}$ (Myr)')
        plt.yscale('log')
        plt.ylim([1.0, 5000.0])
    ...
    def RightPanel():
        ...
        plt.xlabel(r'Stellar mass (M$_\odot$)')
        plt.ylabel(r'Active lifetime for $F_\mathrm{X}$ in HZ (Myr)')
        plt.yscale('log')
        plt.ylim([1.0, 5000.0])
        ...
        for iAge in range(len(Ages)):
            ...
            for iStar in range(cluster.nStars):
                if Fx[iAge, iStar] > Feuv[iAge, iStar] + Fly[iAge, iStar]:
                    ...
                    color.append('cyan')
                elif (Feuv[iAge, iStar] > Fx[iAge, iStar]) and (
                    Feuv[iAge, iStar] > Fly[iAge, iStar]
                ):
                    color.append('green')
            ...
            plt.yscale('log')
            plt.xlabel(r'Stellar Mass (M$_\odot$)')
            plt.ylabel(r'$F_\mathrm{X}$ (erg s$^{-1}$ cm$^{-2}$)')
            plt.xlim([0.08, 1.22])
...
def Fig18():
    ...
    def LowerRight():
        ...
        # Get emitted energies
        energies = cluster.IntegrateEmission(
            AgeMin=1000.0, AgeMax=5000.0, Band='XUV', aOrb='HZ'
        )
