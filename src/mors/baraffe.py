...
class BaraffeTrack:
        ...
        # If Mstar is between two points in the mass grid, need mass interpolation
            ...
            for i in range(29):
                if not Mstar > MassGrid[i + 1]:
                    ...
                    track['Teff'] = (trackp['Teff'] - track['Teff']) * mass_ratio + track[
                        'Teff'
                    ]
                    track['Lstar'] = (trackp['Lstar'] - track['Lstar']) * mass_ratio + track[
                        'Lstar'
                    ]
                    track['Rstar'] = (trackp['Rstar'] - track['Rstar']) * mass_ratio + track[
                        'Rstar'
                    ]
    ...
    def BaraffeSolarConstant(self, tstar, mean_distance):
        ...
        inst = Lstar / (4.0 * np.pi * mean_distance * mean_distance)
