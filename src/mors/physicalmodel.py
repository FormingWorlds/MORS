...
def RotationQuantities(
    ...
    if params['MomentInertiaChangeTorque']:
        StarState['torqueEnvMom'], StarState['torqueCoreMom'] = _torqueMoment(
            StarState, CoreEnvelopeDecoupling
        )
    else:
    ...
    if params['CoreEnvelopeTorque'] and CoreEnvelopeDecoupling:
        StarState['torqueEnvCE'], StarState['torqueCoreCE'] = _torqueCoreEnvelope(
            StarState, params=params
        )
    else:
    ...
    if params['DiskLocking']:
        StarState['torqueEnvDL'] = _torqueDiskLocking(
            StarState, CoreEnvelopeDecoupling, params=params
        )
    else:
    ...
    StarState['torqueEnv'] = (
        StarState['torqueEnvMom']
        + StarState['torqueEnvCG']
        + StarState['torqueEnvWind']
        + StarState['torqueEnvCE']
        + StarState['torqueEnvDL']
    )
    ...
    if CoreEnvelopeDecoupling:
        ...
        StarState['torqueCore'] = (
            StarState['torqueCoreMom'] + StarState['torqueCoreCG'] + StarState['torqueCoreCE']
        )
...
def ExtendedQuantities(
    ...
    StarState['Feuv1HZ'] = StarState['Leuv1'] / (
        4.0 * const.Pi * (aOrbHZ2['HZ'] * const.AU) ** 2.0
    )
    StarState['Feuv2HZ'] = StarState['Leuv2'] / (
        4.0 * const.Pi * (aOrbHZ2['HZ'] * const.AU) ** 2.0
    )
    StarState['FeuvHZ'] = StarState['Leuv'] / (
        4.0 * const.Pi * (aOrbHZ2['HZ'] * const.AU) ** 2.0
    )
...
def _torqueCoreEnvelope(StarState, params=params.paramsDefault):
    ...
    deltaJ = (
        StarState['Icore']
        * StarState['Ienv']
        / (StarState['Icore'] + StarState['Ienv'])
        * (StarState['OmegaCore'] - StarState['OmegaEnv'])
        * const.OmegaSun
    )

    # Get core-envelope coupling timescale in Myr
    timeCE = (
        params['aCoreEnvelope']
        * StarState['OmegaEnv'] ** params['bCoreEnvelope']
        * StarState['Mstar'] ** params['cCoreEnvelope']
    )
...
def _torqueWind(StarState, params=params.paramsDefault):
    ...
    torqueWind = (
        K1Matt**2.0
        * StarState['Bdip'] ** (4.0 * mMatt)
        * StarState['Mdot'] ** (1.0 - 2.0 * mMatt)
        * (StarState['Rstar'] * const.Rsun) ** (4.0 * mMatt + 2.0)
        * StarState['OmegaEnv']
        * const.OmegaSun
        / (
            K2Matt**2.0 * StarState['vEsc'] ** 2.0
            + (StarState['OmegaEnv'] * const.OmegaSun) ** 2.0
            * (StarState['Rstar'] * const.Rsun) ** 2.0
        )
        ** mMatt
    )
...
def _Mdot(StarState, params=params.paramsDefault):
    ...
    if StarState['Ro'] >= params['RoSatMdot']:
        MdotWind = (
            params['MdotSun']
            * StarState['Rstar'] ** 2.0
            * (StarState['Ro'] / const.RoSun) ** params['aMdot']
            * StarState['Mstar'] ** params['bMdot']
        )
    else:
        MdotWind = (
            params['MdotSun']
            * StarState['Rstar'] ** 2.0
            * (params['RoSatMdot'] / const.RoSun) ** params['aMdot']
            * StarState['Mstar'] ** params['bMdot']
        )
...
def MdotFactor(Mstar, Rstar, OmegaSurface, params=params.paramsDefault):
    ...
    if fracBreak > params['fracBreakThreshold']:
        ...
        if fracBreak < 1.009:
            ...
            factor = (
                (5.94401988e03) * fracBreak**7.0
                + (-2.12411847e04) * fracBreak**6.0
                + (3.07933216e04) * fracBreak**5.0
                + (-2.32693067e04) * fracBreak**4.0
                + (9.80417648e03) * fracBreak**3.0
                + (-2.27923380e03) * fracBreak**2.0
                + (2.68629816e02) * fracBreak**1.0
                + (-1.13054867e01) * fracBreak**0.0
            )
...
def OmegaBreak(Mstar, Rstar):
    ...
    return (
        (2.0 / 3.0) ** (3.0 / 2.0)
        * (const.GravConstant * Mstar * const.Msun) ** 0.5
        / (Rstar * const.Rsun) ** (3.0 / 2.0)
        / const.OmegaSun
    )
