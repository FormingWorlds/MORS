...
def EvolveRotation(Mstar=None,Omega0=None,OmegaEnv0=None,OmegaCore0=None,AgeMin=None,AgeMax=None,AgesOut=None,params=params.paramsDefault,StarEvo=None):
    ...
    if Omega0 is None:
        if ( ( OmegaEnv0 is None ) or ( OmegaCore0 is None ) ):
            raise Exception("if Omega0 is not set then both OmegaEnv0 and OmegaCore0 must be set in call to function")
    else:
        if not ( ( OmegaEnv0 is None ) and ( OmegaCore0 is None ) ):
            raise Exception("if Omega0 is set then neither OmegaEnv0 nor OmegaCore0 can be set in call to function")
...
def _shouldAppend(Age,AgesOut):
    ...
    if not AgesOut is None:
        ...
        if isinstance(AgesOut,(float,int)):
            ...
            if not indexMin == 0:
                raise Exception("indexMin is not zero, probably because the input AgesOut is not in ascending order")
