...
def FitRotation(
    Mstar=None, Age=None, Omega=None, AgeMin=None, params=params.paramsDefault, StarEvo=None
):
    ...
    if Omega is None:
        raise Exception('argument Omega must be set in call to function')
...
def EvolveRotation(
    ...
    while Age < 0.999 * AgeMax:
        ...
        if shouldAppend:
            Tracks = _AppendTracks(
                Tracks, Mstar, Age, dAge, OmegaEnv, OmegaCore, params, StarEvo
            )
        ...
        if iAge >= params['nStepMax']:
            raise Exception('too many timesteps taken')
...
def EvolveRotationStep(
    ...
    if (dAgeMax is None) and (dAge is None):
        raise Exception('either dAge or dAgeMax argument must be set in call to function')
    ...
    if params['TimeIntegrationMethod'] == 'ForwardEuler':
        ...
        dAge, dAgeNew, OmegaEnv, OmegaCore = _EvolveRotationStepRBFixed(
            Mstar=Mstar,
            Age=Age,
            OmegaEnv=OmegaEnv,
            OmegaCore=OmegaCore,
...
def _EvolveRotationStepFE(
    ...
    # Get rates of change
    dOmegaEnvdt, dOmegaCoredt = phys.dOmegadt(
        Mstar=Mstar,
        Age=Age,
        OmegaEnv=OmegaEnv,
        OmegaCore=OmegaCore,
...
def _EvolveRotationStepRKF(
    ...
    while True:
        ...
        X6 = (
            X
            + (1631.0 / 55296.0) * dAge * k1
            + (175.0 / 512.0) * dAge * k2
            + (575.0 / 13824.0) * dAge * k3
            + (44275.0 / 110592.0) * dAge * k4
            + (253.0 / 4096.0) * dAge * k5
        )
        k6 = np.array(
            phys.dOmegadt(
                Mstar=Mstar,
        ...
        Xnew = (
            ...
            + (250.0 / 621.0) * dAge * k3
            + (125.0 / 594.0) * dAge * k4
            + (512.0 / 1771.0) * dAge * k6
        )

        # Calculate forth order update
        Xforth = (
            X
            + (2825.0 / 27648.0) * dAge * k1
            + (18575.0 / 48384.0) * dAge * k3
            + (13525.0 / 55296.0) * dAge * k4
            + (277.0 / 14336.0) * dAge * k5
            + (1.0 / 4.0) * dAge * k6
        )
...
def _EvolveRotationStepRB(
    ...
    while True:
        ...
        for i in range(CoefficientsRB['s']):
...
def _EvolveRotationStepRBFixed(
    Mstar=None,
    Age=None,
    OmegaEnv=None,
    OmegaCore=None,
...
def _kCoeffRB(Mstar, Age, dAge, X, Jac, nVar, CoefficientsRB, params, StarEvo):
    ...
    for i in range(1, CoefficientsRB['s']):
        ...
        for i2 in range(i):
            for iVar in range(nVar):
                RateFunction[iVar] += (
                    dAge * CoefficientsRB['gamma'][i, i2] * np.sum(Jac[iVar, :] * kCoeff[:, i2])
                )
...
def _GaussianElimination(Ain, B):
    ...
    for i in range(N):
        ...
        if A[i, i] == 0.0:
            raise Exception('diagonal term in zero')
...
def _CreateTracks(Mstar, Age, dAge, OmegaEnv, OmegaCore, params, StarEvo):
    ...
    if params['ExtendedTracks']:
        ...
        StarState = phys.ExtendedQuantities(
            Mstar=Mstar,
            Age=Age,
            OmegaEnv=OmegaEnv,
            OmegaCore=OmegaCore,
    ...
    Tracks = _AppendTracks(
        Tracks, Mstar, Age, dAge, OmegaEnv, OmegaCore, params, StarEvo, StarState=StarState
    )
...
def _AppendTracks(
    Tracks, Mstar, Age, dAge, OmegaEnv, OmegaCore, params, StarEvo, StarState=None
):
...
def _CheckBadData(Tracks):
    ...
    if Tracks['OmegaEnv'][-1] <= 0.0:
        log.error('negative value of OmegaEnv in rotational evolution calculation')
        error = True
    if Tracks['OmegaCore'][-1] <= 0.0:
        log.error('negative value of OmegaCore in rotational evolution calculation')
        error = True

    # Loop over elements in Tracks and check each one for NaN or Inf
    for track in Tracks:
        if math.isnan(Tracks[track][-1]):
            log.error('NaN found for ' + track + ' in rotational evolution calculation')
            error = True
        if math.isinf(Tracks[track][-1]):
            log.error('inf found for ' + track + ' in rotational evolution calculation')
            error = True

    # If there was an error, kill the code
    if error:
        raise Exception('ending simulation due to bad data found')
